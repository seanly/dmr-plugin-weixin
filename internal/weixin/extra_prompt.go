...
const weixinInboundBuiltinSchedulingHint = `【微信·定时】在本会话使用 cronAdd 时不要传 tape_name，任务绑定当前会话 tape。由 dmr-plugin-cron 到点触发的 RunAgent 没有微信入站上下文，助手最终文本不会自动发到微信；若要让定时/提醒内容到达本单聊，须调用 weixinSendText 并指定 tape_name（与当前会话一致）。一次性提醒可在 cronAdd 中使用 run_once=true。

[Cron/Weixin] In this chat, cronAdd does not take tape_name—the job uses the current session tape. Scheduled runs lack Weixin inbound context—assistant text is not auto-posted to IM; use weixinSendText with tape_name (or peer_id) matching this chat to deliver. For one-shot jobs, use run_once=true on cronAdd.`

const weixinInboundBuiltinReportHint = `【微信·报告】长报告、多段落交付：可用 fsWrite 落盘（如 .md），再用 **weixinSendText** 发摘要，或用 **weixinSendFile** 发送文件（路径或 URL）。

[Weixin reports] For long output: write to a file if needed, summarize via weixinSendText, or deliver the attachment with weixinSendFile (path or http/https URL).`
